...
def send_email_tool(payload: str) -> str:
    """Use this to send a confirmation email to a customer after completing an action like creating a ticket or resolving an order issue. Input must be formatted exactly as: user_id=U00XXX; subject=...; content=..."""
    try:
        base_url = os.getenv("MOCK_API_BASE_URL", "http://localhost:8000")
        data = _parse_email_payload(payload)
        r = httpx.post(f"{base_url}/emails/notify", json=data)
