...
from mock_api.models import UserTable, ProductTable, OrderTable, OrderItemTable, TransactionTable, TicketTable
...
def seed_initial_data(session: Session):
    ...
    if not session.exec(select(UserTable)).first():
        users_data = [
    ...
    if not session.exec(select(ProductTable)).first():
        products_data = [
            ProductTable(product_id="PROD-001", name="Samsung Galaxy A55 5G", category="Mobile Phones", price_vnd=9490000, stock_quantity=15, description="6.6-inch Super AMOLED screen, Exynos 1480, 50MP camera.", warranty_months=12),
            ProductTable(product_id="PROD-002", name="Logitech MX Master 3S", category="Computer Accessories", price_vnd=2190000, stock_quantity=30, description="Wireless performance ergonomic mouse with 8K DPI sensor.", warranty_months=12),
            ProductTable(product_id="PROD-003", name="MacBook Air M3 13-inch", category="Laptops", price_vnd=28990000, stock_quantity=8, description="Apple M3 chip, 8GB RAM, 256GB SSD, Liquid Retina display.", warranty_months=12),
            ProductTable(product_id="PROD-004", name="Sony WH-1000XM5 Headphones", category="Audio", price_vnd=7490000, stock_quantity=12, description="Industry-leading noise canceling wireless headphones.", warranty_months=12),
            ProductTable(product_id="PROD-005", name="Xiaomi Smart Band 9", category="Wearables", price_vnd=890000, stock_quantity=50, description="1.62-inch AMOLED screen, 21-day battery life, health tracking.", warranty_months=12),
            ProductTable(product_id="PROD-006", name="ASUS ROG Zephyrus G14", category="Laptops", price_vnd=34990000, stock_quantity=5, description="AMD Ryzen 9, RTX 4060, 16GB RAM, ROG Nebula Display.", warranty_months=24),
            ProductTable(product_id="PROD-007", name="Keychron Q1 Pro Mechanical Keyboard", category="Computer Accessories", price_vnd=4500000, stock_quantity=20, description="QMK/VIA wireless custom mechanical keyboard, aluminum body.", warranty_months=12),
            ProductTable(product_id="PROD-008", name="iPad Air M2 11-inch", category="Tablets", price_vnd=16990000, stock_quantity=10, description="Apple M2 chip, 11-inch Liquid Retina, Wi-Fi 128GB.", warranty_months=12),
        ]
        session.add_all(products_data)
        session.commit()

    # Seed Orders & Order Items if missing
    if not session.exec(select(OrderTable)).first():
        orders_data = [
            (
                OrderTable(
    ...
    if not session.exec(select(TransactionTable)).first():
        transactions_data = [
            TransactionTable(
    ...
    if not session.exec(select(TicketTable)).first():
        tickets_data = [
            TicketTable(
